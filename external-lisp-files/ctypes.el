...
;; I do not use C++, the probability is high (about 1.2, on a scale
;; from 0 to 1) that I've missed something.)
;;
;; This package provides a number of methods to define new types:
;; * you can specify types explicitly;
;; * parse one buffer;
;; * walk through all buffers;
;; * open a file;
;; * search through all files in a directory hierarchy;
;; * all files refered by a TAGS table, or;
;; * automatically scan for new types in all visited files.
;;
;; The set of known types can be edited using a special editing mode.
;;
;; It is also possible to save all types found to a file for future
;; sessions.
...
;;  (require 'ctypes)
;;
;; Please see the "Example" section below for more elaborate
;; installations.
;;
;; You must, of course, activate font-lock as well.
;;  (add-hook 'c-mode-hook 'turn-on-font-lock)
;;  (add-hook 'c++-mode-hook 'turn-on-font-lock)
;;
;; Or, if you are using a modern version of font-lock, you can
;; activate font-lock on all modes:
;;  (global-font-lock-mode 1)
;;
;; Unfortunately, font-lock in XEmacs is a bit outdated...
...
;;
;; To automatically save the types when Emacs is exited, set the
;; variable `ctypes-write-types-at-exit' to any non-nil value.  Should
;; it be set to the atom `ask' the user is asked before the types
;; are saved.
;;
...
;; before each new type is generated.

;; Delayed Action:
;;
;; Every time the set of known types has changed all loaded buffers
;; must be refontified.  Since this can take some time it is possible
;; to inhibit the fontification when executing a command by preceding
;; it with C-u.
;;
;; When a number of such commands has been issued, the command
;; `ctypes-update' ensures that font-lock has been informed and that
;; the display has been updated.

;; Lazy lock:
;;
;; Every time a new type has been added to ctypes, all buffers in the
;; affected modes must be refontified.  This could take some time if
;; you are editing several large buffers.
;;
;; To avoid this you can activate `lazy-lock' a standard package that
;; will refontify buffers "demand-driven, deferred and stealthy."
;; I also recommend to lower the lazy-lock lower limit since it
;; still can take some time to refontify a large number of small
;; buffers.
;;
;; To install lazy-lock for GNU Emacs, enter the following in your
;; ~/.emacs:
;;
;;  (setq font-lock-support-mode 'lazy-lock-mode)
;;  (setq lazy-lock-minimum-size 2000)
;;
;; XEmacs users must use the following code instead:
...
;; collected during one session will be kept for the next, and
;; all new files will be parsed in the boldly search for new
;; types.
;;
;; (require 'ctypes)
...
;; Example 2:
;;
;; Should you manage one types file per project you can use the
;; following setup.  (The sequence of nil's and t's means "don't
;; inhibit-redraw", "Raise an error if something went wrong when
;; reading the file", and "Don't write anything in the minibuffer
;; when loading the file", respectively.)
;;
;; (requite 'ctypes)
;; (ctypes-read-file "~/src/alpha/.ctypes" nil nil t)
;; (ctypes-read-file "~/src/beta/.ctypes" nil nil t)
;; (ctypes-read-file "~/src/gamma/.ctypes" nil nil t)

;; Example 3:
;;
;; In this example, ctypes will not be not loaded until either c-mode
;; or c++-mode is activated.  When ctypes is loaded, it will read the
;; default ctypes file, all currently loaded files are parsed, and
;; `ctypes-auto-parse-mode' is activated so that all files loaded in
;; the future will be parsed.
;;
...
;; Home Page:
;;
;; You can always find the latest version of `ctypes' on my Emacs
;; page:
;;
;;     http://www.csd.uu.se/~andersl/emacs.shtml
;;
;; While you are visiting, take a look at the other Emacs packages
;; I've written or enhanced during the past years, for example:
;;
;; Follow Mode:  Make two side-by-side windows act as one very tall
;;               window.
;;
;; Folding Mode: Folds buffer at all those strange `{{{' and `}}}'.
;;               (Originally written by Jamie Lokier.)
;;
;; Java Font Lock: My own version of font-lock keywords for Java.
...
Return non-nil if new types are found."
  ...
  (let ((dirs (list dir))
    ...
    (while dirs
      (setq dir (car dirs))
      (setq dirs (cdr dirs))
      (let ((files (directory-files dir t))) ; Files and dirs
	(while files
	  (let ((file (car files)))
	    (cond
...
Return non-nil if any types actually were removed."
  (interactive "P")
  (setq ctypes-parse-error nil)		; Debug
  (let ((desc ctypes-mode-descriptor)
	(modes '()))
    (while desc
      (if (ctypes-set-types (car (car desc)) '())
	  (setq modes (cons (car (car desc)) modes)))
      (setq desc (cdr desc)))
    (if modes
...
is called with no args."
  ...
  ;; Add ourselves to all modes in `ctypes-major-modes'.  We assume
  ;; that all major modes have a corresponding hook.
  (let ((descs ctypes-mode-descriptor)
	mode-hook-symbol)
    (while descs
      (setq mode-hook-symbol
	    (intern (concat (symbol-name (car (car descs))) "-hook")))
      (if ctypes-auto-parse-mode
	  (add-hook mode-hook-symbol 'ctypes-buffer)
	(remove-hook mode-hook-symbol 'ctypes-buffer))
      (setq descs (cdr descs))))
  (if ctypes-auto-parse-mode
...
Return non-nil if new types are found."
  ...
  (let ((current-types-alist '()))
    ...
    (let ((modes '()))		; Updated modes
      (while current-types-alist
	(if (not (ctypes-subset
		  (ctypes-get-types (car (car current-types-alist)))
		  (cdr (car current-types-alist))))
	    ;; New types was defined for this mode.
	    (setq modes (cons (car (car current-types-alist)) modes)))
	(ctypes-add-types (car (car current-types-alist))
			  (cdr (car current-types-alist)))
	(setq current-types-alist (cdr current-types-alist)))
      (if modes
	  (ctypes-perform-action modes t delay-action))
      modes)))
...
is used."
  ...
  (save-excursion
	...
	(insert (prin1-to-string (car (car descs))))
	(insert " '")
	(insert (prin1-to-string (ctypes-get-types (car (car descs)))))
	(insert ")\n\n")
	(setq descs (cdr descs))))
...
Return non-nil if at least one new type was added."
  (setq type-list (reverse type-list))	; Try to keep original order.
  (let ((current-types (ctypes-get-types mode))
	(added nil))
    (while type-list
      (if (member (car type-list) current-types)
	  ()
	(setq current-types (cons (car type-list) current-types))
	(setq added t))
  ...
  (cond (delay-action
	 ...
	 (if changed-p
	     (while modes
	       ...
	       (setq modes (cdr modes)))))
	(t
	 ;; ;; Find modes in the delayed action list.
	 ;; ;; (Uncomment this to perform action for all modes in
	 ;; ;; the delay list.)
	 ;;(while ctypes-delayed-action-list
	 ;;  (if (not (memq (car ctypes-delayed-action-list) modes))
	 ;;      (setq modes (cons (car ctypes-delayed-action-list) modes)))
	 ;;  (setq ctypes-delayed-action-list
	 ;;	 (cdr ctypes-delayed-action-list)))
...
;; TODO: Warning!  We destructively update the list in perform action.
...
;; TODO: Try to check if we have landed in the middle of a comment.

;; The filename is only used to track down errors...
...
See the function `ctypes-tags'."
  (save-excursion
	...
	(let* ((buffer-file-name new)
	  ...
	  (if (assq mode ctypes-mode-descriptor) ; Not really needed...
...
;; The regexp build is not by any means perfect.  See `make-regexp' by
;; Simon Marshall for a better approach.
(defun ctypes-build-regexp (lst)
  "Build regexp recognizing the strings in LST."
  (let ((regexp "")
	(first t))
    (while lst
      (if first
	  (setq regexp (car lst))
	(setq regexp (concat regexp "\\|" (regexp-quote (car lst)))))
      (setq first nil)
      (setq lst (cdr lst)))
    regexp))


(defun ctypes-regexp-depth (keyword)
  ...
  (let ((types (ctypes-get-types mode)))
	...
	(ctypes-font-lock-delete-keywords mode rules)
      (ctypes-font-lock-install-keywords
       mode (ctypes-build-regexp types) rules))
    (ctypes-font-lock-refontify mode)))
